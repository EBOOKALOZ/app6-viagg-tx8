-- ═══════════════════════════════════════════════════════════════════════════
-- Cobrança por clique no botão INTERESSE de frete (9 cr, igual serviços).
-- Espelha charge_service_interest_click: sempre cobra, sem exceção pro dono
-- e sem dedup.
-- ═══════════════════════════════════════════════════════════════════════════

create or replace function public.charge_freight_interest_click(
  p_listing_id uuid,
...
begin
  ...
  from public.freight_listings where id = p_listing_id;
  ...
  where feature_code = 'freight_interest_click'
  limit 1;
...
grant execute on function public.charge_freight_interest_click(uuid, text) to anon, authenticated;

insert into public.merchant_credit_usage_rules (feature_code, feature_name, credits_cost, is_active)
select 'freight_interest_click', 'Clique em Interesse (frete)', 9, true
where not exists (
  select 1 from public.merchant_credit_usage_rules where feature_code = 'freight_interest_click'
);
