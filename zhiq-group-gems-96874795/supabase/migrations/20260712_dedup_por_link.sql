-- ============================================================
-- RADAR IA — DEDUPLICAÇÃO SOMENTE POR LINK (2026-07-12)
--
-- Correção de rota: o identificador ÚNICO e permanente do grupo é o
-- LINK (lower/trim) — nunca o nome. Grupos com nome igual e links
-- diferentes são grupos DISTINTOS (falso positivo da versão anterior).
--
-- Camadas de unicidade por link (todas já ativas, mantidas):
--  1. ÍNDICE ÚNICO parcial uq_whatsapp_groups_link_ativo
--     (lower(trim(group_link)) WHERE is_active) — impossível o mesmo
--     link estar ativo 2x, por QUALQUER origem;
--  2. Trigger de exclusividade — mensagem amigável antes do índice;
--  3. Pré-checagem no formulário.
--
-- O flag 'duplicado' do motor volta a olhar SÓ o link (cinto extra
-- contra corrida de inserção). Reprocesso libera os falsos positivos
-- vetados por nome. Idempotente.
-- ============================================================
...
BEGIN
  WITH base AS (
    SELECT g.id,
      ...
      -- DUPLICADO: SÓ pelo LINK normalizado (identificador real) contra
      -- grupo ATIVO mais antigo. Nome igual NÃO é duplicidade.
      EXISTS (
  ...
  SELECT c.id, c.score,
    ...
    c.factors, 'rules-v3-link-dedup', now()
  FROM calc c
...
-- Reprocesso: libera os falsos positivos (nome igual, link diferente)
