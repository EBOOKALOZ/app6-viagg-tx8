-- ═══════════════════════════════════════════════════════════════════════════
-- Comando Convênio · Fase 1 completa + "Quero ser parceiro" (partner leads)
--
-- A migration 20260801_convenio_fase1_schema.sql nunca foi confirmada como
-- aplicada no banco real (types.ts gerado não contém nenhuma tabela
-- convenio_*). Esta migration:
--   1) Reaplica o schema INTEIRO da Fase 1 (idêntico ao arquivo original —
--      11 tabelas convenio_*, papel gestor_convenio, permissões convenio:*,
--      função is_gestor_convenio(), RLS). Todo o bloco é idempotente
--      (IF NOT EXISTS/ON CONFLICT), seguro mesmo que parte já exista.
--   2) Cria convenio_partner_leads — tabela nova para o formulário público
--      do botão "Quero ser parceiro" em /medprev. RLS: INSERT público
--      (anon + authenticated) apenas; SELECT/UPDATE/DELETE restritos a
--      is_gestor_convenio() (Super Painel do Gestor).
--
...
-- ─────────────────────────────────────────────────────────────────────────
-- 12. convenio_partner_leads — captação do botão "Quero ser parceiro"
--     (novo nesta migration — não faz parte do schema original da Fase 1)
-- ─────────────────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS public.convenio_partner_leads (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  nome             TEXT NOT NULL CHECK (char_length(btrim(nome)) BETWEEN 2 AND 150),
  instituicao      TEXT CHECK (instituicao IS NULL OR char_length(instituicao) <= 150),
  tipo_parceiro    TEXT NOT NULL CHECK (tipo_parceiro IN (
                     'hospital', 'clinica', 'laboratorio', 'farmacia', 'consultorio',
                     'plano_saude', 'empresa', 'associacao', 'ong', 'outro'
                   )),
  cidade           TEXT NOT NULL CHECK (char_length(btrim(cidade)) BETWEEN 2 AND 100),
  estado           TEXT NOT NULL CHECK (estado ~ '^[A-Za-z]{2}$'),
  whatsapp         TEXT NOT NULL CHECK (char_length(regexp_replace(whatsapp, '\D', '', 'g')) BETWEEN 10 AND 11),
...
COMMENT ON TABLE public.convenio_partner_leads IS
'Comando Convênio: leads captados pelo botão "Quero ser parceiro" da página pública /medprev. Ciclo de vida via status (novo → em_analise → contatado → aprovado/recusado), gerenciado no Super Painel do Gestor.';

CREATE INDEX IF NOT EXISTS idx_convenio_partner_leads_status     ON public.convenio_partner_leads (status);
CREATE INDEX IF NOT EXISTS idx_convenio_partner_leads_created_at ON public.convenio_partner_leads (created_at DESC);

CREATE OR REPLACE FUNCTION public.convenio_partner_leads_set_updated_at()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_convenio_partner_leads_updated_at ON public.convenio_partner_leads;
CREATE TRIGGER trg_convenio_partner_leads_updated_at
  BEFORE UPDATE ON public.convenio_partner_leads
  FOR EACH ROW
  EXECUTE FUNCTION public.convenio_partner_leads_set_updated_at();

-- ─────────────────────────────────────────────────────────────────────────
-- 13. RLS — leitura pública apenas do que é institucional/transparência;
...
ALTER TABLE public.convenio_partner_leads       ENABLE ROW LEVEL SECURITY;
...
-- Partner leads: INSERT público (o formulário "Quero ser parceiro" grava sem
-- autenticação); NENHUMA policy de SELECT pública é criada — visitantes não
-- podem ler leads (nem os próprios), somente o Gestor via policy abaixo.
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'convenio_partner_leads' AND policyname = 'convenio_partner_leads_insert_public'
  ) THEN
    CREATE POLICY "convenio_partner_leads_insert_public" ON public.convenio_partner_leads
      FOR INSERT
...
-- Defesa em profundidade: o schema public deste projeto tem uma default ACL
-- que concede SELECT automático a anon em toda tabela nova (não usada pelas
-- demais tabelas do módulo, criadas antes dessa ACL existir). Revoga
-- explicitamente para não depender só do RLS, e concede apenas o INSERT que
-- o formulário público realmente precisa.
...
-- is_gestor_convenio() em todas as tabelas do módulo (inclui partner_leads).
DO $$
DECLARE
  t TEXT;
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    'convenio_ai_notes', 'convenio_settings', 'convenio_partner_leads'
  ]
  LOOP
    EXECUTE format(
...
DO $$ BEGIN
  RAISE NOTICE '✅ Comando Convênio — Fase 1 completa reaplicada (idempotente) + convenio_partner_leads criada. 12 tabelas convenio_*, papel gestor_convenio, função is_gestor_convenio(), RLS de insert público para leads de parceiro.';
END $$;
