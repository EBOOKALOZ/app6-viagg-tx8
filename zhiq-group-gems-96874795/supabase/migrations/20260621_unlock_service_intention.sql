-- ═══════════════════════════════════════════════════════════════════════════
-- RPC unlock_service_intention — desbloqueia o contato de um lead de SERVIÇO
-- debitando a CARTEIRA PRÓPRIA de serviços (service_credit_balances),
-- NÃO a do lojista/anunciante (espelha unlock_vehicle_intention).
--
-- Custo FIXO de desbloqueio de WhatsApp, configurado no admin em
-- merchant_credit_usage_rules (feature_code = 'service_unlock_whatsapp').
...
create or replace function public.unlock_service_intention(p_intention_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if v_module <> 'services' then
    return jsonb_build_object('success', false, 'error', 'not_service');
  end if;
...
grant execute on function public.unlock_service_intention(uuid) to authenticated;

-- Regra de custo de desbloqueio p/ serviços (default 9) — editável no admin depois.
insert into public.merchant_credit_usage_rules (feature_code, feature_name, credits_cost, is_active)
select 'service_unlock_whatsapp', 'Desbloqueio de WhatsApp (serviço)', 9, true
where not exists (
  select 1 from public.merchant_credit_usage_rules where feature_code = 'service_unlock_whatsapp'
);
