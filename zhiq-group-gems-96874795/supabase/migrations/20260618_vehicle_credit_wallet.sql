-- ═══════════════════════════════════════════════════════════════════════════
-- CARTEIRA DE CRÉDITOS DE VEÍCULOS (própria, separada de imóveis e do marketplace)
-- Espelha o modelo de imóveis (real_estate_*).
--
--   • vehicle_credit_balances  — saldo por owner_user_id (pode ficar NEGATIVO = dívida)
--   • vehicle_credit_ledger    — histórico (clique, desbloqueio, compra, ajuste)
--   • vehicle_credit_purchases — compras de pacote de veículos
--   • charge_vehicle_listing_click() — cobra por clique no anúncio (dívida sem pacote)
--   • vehicle_listing_click_log — dedup anti-spam
--   • pay_grant_legacy()       — ganha o ramo 'vehicle' (credita a carteira de veículos)
--
-- Custo do clique vem de merchant_credit_usage_rules (feature_code
-- 'vehicle_listing_click', default 6). Idempotente.
-- ═══════════════════════════════════════════════════════════════════════════
...
create index if not exists idx_vehicle_credit_ledger_owner
  on public.vehicle_credit_ledger(owner_user_id, created_at desc);
...
create index if not exists idx_vehicle_click_log_listing_fp
  on public.vehicle_listing_click_log(listing_id, fingerprint, created_at desc);

-- ── RLS: dono lê/gerencia sua própria carteira/histórico/compras ──
alter table public.vehicle_credit_balances  enable row level security;
alter table public.vehicle_credit_ledger    enable row level security;
alter table public.vehicle_credit_purchases enable row level security;
alter table public.vehicle_listing_click_log enable row level security;

drop policy if exists vcb_owner_read on public.vehicle_credit_balances;
create policy vcb_owner_read on public.vehicle_credit_balances for select to authenticated using (owner_user_id = auth.uid());

drop policy if exists vcl_owner_read on public.vehicle_credit_ledger;
create policy vcl_owner_read on public.vehicle_credit_ledger for select to authenticated using (owner_user_id = auth.uid());

drop policy if exists vcp_owner_read on public.vehicle_credit_purchases;
create policy vcp_owner_read on public.vehicle_credit_purchases for select to authenticated using (owner_user_id = auth.uid());

-- ═══════════════════════════════════════════════════════════════════════════
-- Cobrança por CLIQUE no anúncio de veículo (igual imóveis: dívida sem pacote).
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if p_fingerprint is not null and exists (
    select 1 from public.vehicle_listing_click_log
...
-- ═══════════════════════════════════════════════════════════════════════════
-- pay_grant_legacy: acrescenta o ramo 'vehicle' (credita vehicle_credit_balances).
-- Recria a função preservando os ramos existentes (merchant/advertiser*/real_estate).
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ─── VEÍCULOS (novo): credita a carteira própria de veículos ───────────────
...
-- Regra de custo de clique p/ veículos (default 6) — editável no admin depois.
-- Sem ON CONFLICT (a tabela pode não ter unique em feature_code): insere só se faltar.
insert into public.merchant_credit_usage_rules (feature_code, feature_name, credits_cost, is_active)
select 'vehicle_listing_click', 'Clique no anúncio de veículo', 6, true
where not exists (
  select 1 from public.merchant_credit_usage_rules where feature_code = 'vehicle_listing_click'
);
