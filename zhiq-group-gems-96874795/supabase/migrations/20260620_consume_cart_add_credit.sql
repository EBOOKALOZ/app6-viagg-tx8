-- ═══════════════════════════════════════════════════════════════════════════
-- Cobrança ao lojista quando um visitante adiciona um produto à cesta
-- (CREDIT_COSTS.visitor_cart_add = 5cr). Espelha consume_marketplace_product_click
-- (owner-skip, debita do pool advertiser > merchant, fire-and-forget).
-- ═══════════════════════════════════════════════════════════════════════════

create table if not exists public.cart_add_click_events (
...
create or replace function public.consume_cart_add_credit(
  p_store_id uuid,
  p_product_id uuid
)
returns jsonb
...
begin
  ...
  where feature_code = 'visitor_cart_add'
  limit 1;
  ...
  select user_id into v_owner_id from public.merchant_stores where id = p_store_id;
  ...
  if v_visitor_id is not null and v_visitor_id = v_owner_id then
    insert into public.cart_add_click_events (store_id, product_id, visitor_user_id, status, credits_charged)
    values (p_store_id, p_product_id, v_visitor_id, 'owner_skip', 0);
    return jsonb_build_object('charged', false, 'reason', 'owner_self_view');
  end if;

  select id into v_advertiser_id from public.advertiser_accounts where user_id = v_owner_id limit 1;
  ...
  if v_pool is null then
    select available_credits, coalesce(consumed_credits, 0) into v_balance, v_consumed
    from public.merchant_credit_balances where store_id = p_store_id for update;
    if v_balance is not null and v_balance >= v_cost then
      v_pool := 'merchant';
    end if;
  end if;
  ...
  if v_pool is null then
    insert into public.cart_add_click_events (store_id, product_id, visitor_user_id, status, credits_charged)
    values (p_store_id, p_product_id, v_visitor_id, 'insufficient_balance', 0);
  ...
  if v_pool = 'advertiser' then
    ...
    values
      (v_advertiser_id, 'debit', v_cost, v_balance, v_new_balance, 'visitor_cart_add',
       'Produto adicionado à cesta — ' || v_cost || ' créditos',
       jsonb_build_object('store_id', p_store_id, 'product_id', p_product_id, 'visitor_user_id', v_visitor_id),
       'consumption');
  else
    ...
    values
      (p_store_id, 'debit', v_cost, v_balance, v_new_balance, 'visitor_cart_add',
       'Produto adicionado à cesta — ' || v_cost || ' créditos',
       jsonb_build_object('product_id', p_product_id, 'visitor_user_id', v_visitor_id));
  end if;

  insert into public.cart_add_click_events (store_id, product_id, visitor_user_id, status, credits_charged)
  values (p_store_id, p_product_id, v_visitor_id, 'charged', v_cost);
...
grant execute on function public.consume_cart_add_credit(uuid, uuid) to anon, authenticated;

insert into public.merchant_credit_usage_rules (feature_code, feature_name, credits_cost, is_active)
select 'visitor_cart_add', 'Produto adicionado à cesta', 5, true
where not exists (
  select 1 from public.merchant_credit_usage_rules where feature_code = 'visitor_cart_add'
);
