-- ═══════════════════════════════════════════════════════════════
-- E-mail de confirmação quando o ANUNCIANTE DE VIAGENS compra um pacote.
-- Viagg-TX8 Platform
--
-- Espelha notify_freight_on_credit_purchase. A compra de pacote de VIAGENS
-- usa walletContext='travel' → metadata.grant_kind='travel' e
-- product_type='travel_credits'. Sem este trigger, a compra de viagens
...
create or replace function public.notify_travel_on_credit_purchase()
returns trigger
language plpgsql
security definer
...
exception
  when others then
    raise warning 'notify_travel_on_credit_purchase falhou: %', sqlerrm;
    return NEW;
end;
$$;

drop trigger if exists trg_notify_travel_on_credit_purchase on public.pay_payment_orders;
create trigger trg_notify_travel_on_credit_purchase
  after update on public.pay_payment_orders
  for each row
  when (
    NEW.status = 'paid'
    and OLD.status is distinct from NEW.status
    and NEW.metadata->>'grant_kind' = 'travel'
  )
  execute function public.notify_travel_on_credit_purchase();
