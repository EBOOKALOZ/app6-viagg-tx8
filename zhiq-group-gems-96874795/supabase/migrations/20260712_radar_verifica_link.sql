-- ============================================================
-- RADAR IA — VERIFICAÇÃO AUTOMÁTICA DO LINK (Nível 1) — 2026-07-12
--
-- A edge radar-ia agora consulta a PRÉVIA PÚBLICA do convite
-- (chat.whatsapp.com/...) e grava o resultado no grupo:
--   • link_status: 'ativo' | 'revogado' | 'erro'
--   • real_name:   nome REAL do grupo (og:title da prévia)
--   • photo_url:   foto do grupo (og:image)
--   • link_verified_at
--
-- O MOTOR passa a usar isso:
--   • link revogado  → veto 'link_invalido' DE VERDADE (não só formato)
--   • nome real muito diferente do cadastrado → 'grupo_suspeito'
-- Limite conhecido: a prévia NÃO expõe contagem de membros (só o
-- Nível 2 — bot dentro do grupo — resolve isso). Idempotente.
-- ============================================================
...
BEGIN
  WITH base AS (
    SELECT g.id,
      ...
      -- LINK INVÁLIDO: formato errado OU prévia confirmou REVOGADO
      ...
      -- NOME DIVERGENTE: prévia trouxe nome real que não bate com o cadastrado
      (g.real_name IS NOT NULL AND NULLIF(TRIM(g.group_name),'') IS NOT NULL
       AND position(lower(trim(g.group_name)) in lower(g.real_name)) = 0
       AND position(lower(g.real_name) in lower(trim(g.group_name))) = 0) AS nome_divergente,
    ...
    SELECT b.id,
      CASE
        ...
        ELSE LEAST(100,
          ...
          + CASE WHEN b.link_verificado THEN 5 ELSE 0 END)  -- bônus: link auditado ✓
      END AS score,
      LEAST(100,
  ...
  SELECT c.id, c.score,
    ...
    c.factors, 'rules-v4-link-verify', now()
  FROM calc c
