-- ═══════════════════════════════════════════════════════════════════════════
-- CARTEIRA DE CRÉDITOS DE SERVIÇOS (própria, separada de imóveis/veículos/marketplace)
-- Espelha o modelo de veículos (vehicle_credit_*).
--
--   • service_credit_balances  — saldo por owner_user_id (pode ficar NEGATIVO = dívida)
--   • service_credit_ledger    — histórico (clique, desbloqueio, compra, ajuste)
--   • service_credit_purchases — compras de pacote de serviços
--   • charge_service_listing_click() — cobra por clique no anúncio (dívida sem pacote)
--   • service_listing_click_log — dedup anti-spam
--   • pay_grant_legacy()       — ganha o ramo 'service' (credita a carteira de serviços)
--
-- Custo do clique vem de merchant_credit_usage_rules (feature_code
-- 'service_listing_click', default 6). Idempotente.
-- ═══════════════════════════════════════════════════════════════════════════

-- ── Saldo ──
create table if not exists public.service_credit_balances (
...
create index if not exists idx_service_credit_ledger_owner
  on public.service_credit_ledger(owner_user_id, created_at desc);

-- ── Compras de pacote ──
create table if not exists public.service_credit_purchases (
  id uuid primary key default gen_random_uuid(),
  owner_user_id uuid not null,
  package_id uuid,
...
create table if not exists public.service_listing_click_log (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists idx_service_click_log_listing_fp
  on public.service_listing_click_log(listing_id, fingerprint, created_at desc);

-- ── RLS: dono lê/gerencia sua própria carteira/histórico/compras ──
alter table public.service_credit_balances  enable row level security;
...
alter table public.service_listing_click_log enable row level security;

drop policy if exists scb_owner_read on public.service_credit_balances;
create policy scb_owner_read on public.service_credit_balances for select to authenticated using (owner_user_id = auth.uid());

drop policy if exists scl_owner_read on public.service_credit_ledger;
create policy scl_owner_read on public.service_credit_ledger for select to authenticated using (owner_user_id = auth.uid());

drop policy if exists scp_owner_read on public.service_credit_purchases;
create policy scp_owner_read on public.service_credit_purchases for select to authenticated using (owner_user_id = auth.uid());

-- ═══════════════════════════════════════════════════════════════════════════
-- Cobrança por CLIQUE no anúncio de serviço (igual veículos: dívida sem pacote).
-- ═══════════════════════════════════════════════════════════════════════════
create or replace function public.charge_service_listing_click(
  p_listing_id uuid,
...
declare
  ...
  v_after   integer;
begin
  select owner_user_id into v_owner from public.service_listings where id = p_listing_id;
  ...
  if p_fingerprint is not null and exists (
    select 1 from public.service_listing_click_log
  ...
  where feature_code = 'service_listing_click' limit 1;
  if v_cost is null then v_cost := 6; end if;
  if v_active is false then
    insert into public.service_listing_click_log (listing_id, fingerprint, charged)
  ...
  from public.service_credit_balances where owner_user_id = v_owner for update;
...
grant execute on function public.charge_service_listing_click(uuid, text) to anon, authenticated;

-- ═══════════════════════════════════════════════════════════════════════════
-- pay_grant_legacy: acrescenta o ramo 'service' (credita service_credit_balances).
-- Recria a função preservando TODOS os ramos existentes (merchant/advertiser*/
-- real_estate/vehicle), copiados verbatim de 20260618_vehicle_credit_wallet.sql.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ─── SERVIÇOS (novo): credita a carteira própria de serviços ──────────────
...
-- Regra de custo de clique p/ serviços (default 6) — editável no admin depois.
insert into public.merchant_credit_usage_rules (feature_code, feature_name, credits_cost, is_active)
select 'service_listing_click', 'Clique no anúncio de serviço', 6, true
where not exists (
  select 1 from public.merchant_credit_usage_rules where feature_code = 'service_listing_click'
);
