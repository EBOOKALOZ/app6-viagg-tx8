/**
 * AdvertiserServicesPage — /anunciante/servicos
 *
 * Painel RESUMIDO do anunciante de serviços. Espelha AdvertiserVeiculosPage,
 * porém com tema violeta. Lista os serviços da conta com miniatura, status
 * e atalho de edição.
 */
...
import { Briefcase, PlusCircle, List, MessageSquare, Coins, Pencil, Loader2, ArrowRight, Eye, TrendingUp, Heart, type LucideIcon } from "lucide-react";
import { resolveServiceTypeLabel } from "@/lib/services/serviceCategories";

interface Action { to: string; icon: LucideIcon; title: string; desc: string; }

const ACTIONS: Action[] = [
  { to: "/anunciante/servicos/anuncios/novo/servico", icon: PlusCircle, title: "Anunciar serviço", desc: "Publique um novo serviço" },
  { to: "/anunciante/servicos/mensagens", icon: MessageSquare, title: "Interessados", desc: "Mensagens e contatos recebidos" },
  { to: "/anunciante/servicos/creditos", icon: Coins, title: "Créditos", desc: "Compre créditos p/ desbloquear contatos" },
];

export default function AdvertiserServicesPage() {
  const { user } = useAuth();
  const navigate = useNavigate();

  const { data: servicos = [], isLoading } = useQuery({
    queryKey: ["servicos-painel-lista", user?.id],
    enabled: !!user?.id,
  ...
  // Saldo de créditos PRÓPRIO de serviços (service_credit_balances por owner_user_id).
  const { data: saldo = 0 } = useQuery({
    queryKey: ["servicos-painel-saldo", user?.id],
    enabled: !!user?.id,
  ...
  // Visitas e Interesses REAIS por anúncio (eventos no ledger de serviços).
  const { data: stats = { visits: {} as Record<string, number>, interests: {} as Record<string, number> } } = useQuery({
    queryKey: ["servicos-visitas-map", user?.id],
    enabled: !!user?.id,
    refetchInterval: 20_000,
    refetchOnMount: "always",
    refetchOnWindowFocus: true,
    queryFn: async () => {
      const { data } = await (supabase.from("service_credit_ledger" as any)
  ...
  // Mensagens AGUARDANDO desbloqueio (leads de serviço pendentes).
  const { data: pendingMsgs = 0 } = useQuery({
    queryKey: ["servicos-painel-pendentes", user?.id],
    enabled: !!user?.id,
    refetchInterval: 15_000,
    refetchOnMount: "always",
    refetchOnWindowFocus: true,
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-1 sm:px-4 py-6 space-y-6 min-w-0">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black text-zinc-900">Painel de Serviços</h1>
          <p className="text-sm text-zinc-500">Anuncie seus serviços e fale com os interessados</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white overflow-hidden">
        <div className="flex items-center justify-between gap-3 p-4 border-b border-zinc-100">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="font-bold text-zinc-900 text-sm">Meus serviços ({servicos.length})</p>
              <p className="text-xs text-zinc-500">Veja e edite seus anúncios</p>
            </div>
          </div>
          <Link
            to="/anunciante/servicos/meus-anuncios"
            className="text-[11px] font-black uppercase tracking-wide text-violet-700 bg-violet-50 hover:bg-violet-100 rounded-lg px-3 py-2 flex items-center gap-1 shrink-0"
          >
        ...
        {isLoading ? (
          <div className="flex items-center gap-2 py-10 px-4 text-zinc-400">
            <Loader2 className="w-5 h-5 animate-spin" /> Carregando seus serviços...
          </div>
        ) : servicos.length === 0 ? (
          <div className="p-8 text-center space-y-3">
            <p className="text-sm text-zinc-400">Você ainda não tem serviços cadastrados.</p>
            <button
              onClick={() => navigate("/anunciante/servicos/anuncios/novo/servico")}
              className="inline-flex items-center gap-2 bg-violet-600 hover:bg-violet-700 text-white font-bold text-sm rounded-xl px-4 py-2"
            >
              <PlusCircle className="w-4 h-4" /> Anunciar serviço
            </button>
          </div>
        ) : (
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white overflow-hidden">
        <div className="flex items-center justify-between gap-3 p-4 border-b border-zinc-100">
          ...
          <div className="flex flex-col items-end shrink-0">
            <span className="text-xl font-black text-violet-600 tabular-nums flex items-center gap-1.5" title="Total de visitas">
        ...
        {servicos.length === 0 ? (
          <p className="p-4 text-sm text-zinc-400 text-center">Sem dados ainda — publique um serviço para começar.</p>
        ) : (
          <div className="divide-y divide-zinc-100">
            {servicos.map((s: any) => (
              <div key={s.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
                <p className="text-sm text-zinc-700 truncate">{s.title || "Serviço"}</p>
        ...
        <div className="flex items-center justify-between gap-3 px-4 py-3 bg-zinc-50 border-t border-zinc-100">
          <span className="text-[11px] font-black uppercase tracking-widest text-zinc-500 flex items-center gap-1.5">
            <Coins className="w-3.5 h-3.5 text-violet-500" /> Saldo atual de créditos
          </span>
          <span className={cn("text-lg font-black tabular-nums", saldo < 0 ? "text-red-600" : "text-violet-600")}>{saldo} cr</span>
        </div>
      </div>
        ...
        {ACTIONS.map((a) => {
          ...
          return (
            <div key={a.to} className="relative mt-3 sm:mt-0">
              <Link
                to={a.to}
                className="relative flex flex-col items-center text-center gap-2 sm:flex-row sm:items-start sm:text-left sm:gap-3 p-4 rounded-2xl border border-zinc-200 bg-white hover:border-violet-300 hover:shadow-sm transition-all h-full"
              >
      ...
      <div className="flex items-start gap-2 bg-violet-50 rounded-2xl p-3 border border-violet-100">
        <Coins className="w-4 h-4 text-violet-500 mt-0.5 shrink-0" />
