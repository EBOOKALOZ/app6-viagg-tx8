 ...
 *
 * Hiper-hook administrativo para a página "Fretes & Transportes" do admin.
 * Espelha useAdminServiceOverview. Agrega dados reais de:
 *   - freight_listings
 *   - freight_listing_contacts
 *   - freight_media
 *   - real_estate_credit_packages / purchases (category='freight')
 *   - advertiser_credit_ledger (filtrando módulo freight)
 *   - advertiser_contact_intentions (módulo freight)
...
export interface AdminFreightListingRow {
  id: string;
  title: string;
...
export interface AdminFreightAdvertiser {
  user_id: string;
  name: string;
  kind: AdvertiserKind;
  city: string | null;
...
export interface AdminFreightPackageStats {
  id: string;
  name: string;
  slug: string;
  price_brl: number;
...
export interface AdminFreightLedgerEntry {
  id: string;
...
export interface AdminFreightGrowthPoint {
  day: string;
  listings: number;
  advertisers: number;
}

export interface AdminFreightKpis {
  total_listings: number;
  active_listings: number;
  pending_listings: number;
...
export interface UseAdminFreightOverviewOptions {
  periodDays?: number;
}
...
export function useAdminFreightOverview(opts: UseAdminFreightOverviewOptions = {}) {
  const periodDays = opts.periodDays ?? 30;

  const query = useQuery({
    queryKey: ["admin-freight-overview", periodDays],
    staleTime: 30_000,
    refetchOnWindowFocus: false,
    queryFn: async () => {
      // ── 1. Listings ─────────────────────────────────────────────────────
      const { data: listingsRaw } = await (supabase.from("freight_listings") as unknown)
        .select(
          "id, title, vehicle_type, visibility_status, price_label, is_featured, created_at, updated_at, published_at, city, state, neighborhood, owner_user_id"
        )
      ...
      // ── 3. Pacotes + compras (real_estate_credit_packages category='freight') ──
      const { data: freightPkgs } = await (supabase.from("real_estate_credit_packages") as unknown)
        .select("*")
        .eq("category", "freight")
        .order("sort_order", { ascending: true });

      const freightPackagesDb: unknown[] = freightPkgs || [];

      const freightPackageIds = freightPackagesDb.map((p) => p.id);
      ...
      const purchases = allPurchases.filter((p) => freightPackageIds.includes(p.package_id));

      const userPackageMap = new Map<string, { name: string | null; slug: string | null; count: number }>();
      for (const p of purchases) {
        const pkg = freightPackagesDb.find((k: unknown) => k.id === p.package_id);
      ...
      // ── 4. Ledger de créditos (filtrar módulo freight) ──────────────────
      ...
      const ledgerFreight = ledgerAll.filter((e) => {
        const hay = `${e.module || ""} ${e.reason_code || ""} ${e.description || ""}`.toLowerCase();
        return (
          hay.includes("freight") ||
          hay.includes("frete") ||
          !e.module
        );
      });
      ...
      // ── 5. Leads (advertiser_contact_intentions) p/ freight ─────────────
      ...
      const rows: AdminFreightListingRow[] = listings.map((l) => {
      ...
      const advertisers: AdminFreightAdvertiser[] = Array.from(listingsCountByUser.keys()).map((uid) => {
      ...
      const packageStats: AdminFreightPackageStats[] = freightPackagesDb.map((p: unknown) => {
      ...
      const ledger: AdminFreightLedgerEntry[] = ledgerFreight.slice(0, 300).map((e) => {
      ...
      const growth: AdminFreightGrowthPoint[] = daysWindow.map((day) => {
