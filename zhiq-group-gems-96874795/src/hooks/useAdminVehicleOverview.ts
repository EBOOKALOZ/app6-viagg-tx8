 ...
 *
 * Hiper-hook administrativo para a página "Veículos" do admin.
 * Agrega dados reais de:
 *   - vehicle_listings
 *   - vehicle_listing_contacts
 *   - vehicle_media
 *   - advertiser_credit_packages / purchases (módulo vehicle)
 *   - advertiser_credit_ledger (filtrando módulo vehicle)
 *   - advertiser_contact_intentions (módulo vehicle)
...
export interface AdminVehicleListingRow {
  id: string;
  title: string;
...
export interface AdminVehicleAdvertiser {
  user_id: string;
  name: string;
  kind: AdvertiserKind;
  city: string | null;
...
export interface AdminVehiclePackageStats {
  id: string;
  name: string;
  slug: string;
  price_brl: number;
...
export interface AdminVehicleLedgerEntry {
  id: string;
...
export interface AdminVehicleGrowthPoint {
  day: string;
  listings: number;
  advertisers: number;
}

export interface AdminVehicleKpis {
  total_listings: number;
  active_listings: number;
  pending_listings: number;
...
export interface UseAdminVehicleOverviewOptions {
  periodDays?: number;
}
...
export function useAdminVehicleOverview(opts: UseAdminVehicleOverviewOptions = {}) {
  const periodDays = opts.periodDays ?? 30;

  const query = useQuery({
    queryKey: ["admin-vehicle-overview", periodDays],
    staleTime: 30_000,
    ...
    queryFn: async () => {
      ...
      const rows: AdminVehicleListingRow[] = listings.map((l) => {
      ...
      const advertisers: AdminVehicleAdvertiser[] = Array.from(listingsCountByUser.keys()).map((uid) => {
      ...
      const ledger: AdminVehicleLedgerEntry[] = ledgerVehicle.slice(0, 300).map((e) => {
      ...
      const growth: AdminVehicleGrowthPoint[] = daysWindow.map((day) => {
