...
interface FreightImageUploadProps {
  listingId?: string;
  onUploadComplete?: (mediaId: string, path: string) => void;
  onFilesSelected?: (files: File[]) => void;
}

export const FreightImageUpload: React.FC<FreightImageUploadProps> = ({
  listingId,
  onUploadComplete,
  onFilesSelected
}) => {
  ...
  const uploadFiles = async (fileArray: File[]) => {
    try {
      ...
      if (!listingId) throw new Error("ID do frete é obrigatório para upload imediato");
      ...
      for (let i = 0; i < fileArray.length; i++) {
        ...
        const filePath = `${user.id}/freight/${listingId}/original/${fileName}`;
        const blob = processed.blob;
        console.log(`[FreightUpload] ${file.name}: ${formatFileSize(file.size)} → ${formatFileSize(blob.size)}`);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {uploadedImages.map((img) => (
              <div key={img.id} className="relative aspect-square rounded-2xl overflow-hidden bg-white border border-blue-50 shadow-sm group">
                <img src={img.path} className="absolute inset-0 w-full h-full object-cover" alt="frete" />
