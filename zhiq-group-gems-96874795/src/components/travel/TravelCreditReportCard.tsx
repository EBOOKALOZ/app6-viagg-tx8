/**
 * TravelCreditReportCard — Relatório de créditos de VIAGENS & TURISMO.
 * Espelha FreightCreditReportCard.
 */
...
export function TravelCreditReportCard() {
  const { user } = useAuth();

  const { data: balance = 0 } = useQuery({
    queryKey: ["tv-report-balance", user?.id],
    enabled: !!user?.id,
  ...
  const { data: debits = [], isLoading: debitsLoading } = useQuery({
    queryKey: ["tv-report-debits", user?.id],
    enabled: !!user?.id,
    refetchInterval: 20_000,
    queryFn: async () => {
      ...
      return (data || []).filter(
        (e: any) =>
          e?.metadata?.event === "listing_click" ||
          e?.metadata?.event === "interest_click" ||
          e?.metadata?.event === "feature_listing" ||
          e?.entry_type === "debit_unlock" ||
          e?.entry_type === "expired"
      );
    },
  });

  const { data: purchases = [], isLoading: purchasesLoading } = useQuery({
    queryKey: ["tv-report-purchases", user?.id],
    enabled: !!user?.id,
    refetchInterval: 30_000,
    queryFn: async () => {
      const { data } = await (supabase.from("travel_credit_purchases") as any)
    ...
    <section className="space-y-6">
      <div className="space-y-3">
        ...
        <p className="text-[#A7B0BE] font-bold uppercase text-[10px] tracking-[0.2em] ml-16">
          Débitos de navegação, interesses e compras de pacotes (viagens)
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-[#1B1F24] border-[#2A3038]">
          <CardContent className="p-5 space-y-1">
            <p className="text-[10px] font-black uppercase tracking-widest text-[#A7B0BE] flex items-center gap-1.5">
              <Wallet className="w-3.5 h-3.5 text-sky-400" /> Saldo atual
            </p>
...
export default TravelCreditReportCard;
