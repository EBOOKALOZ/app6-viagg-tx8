/**
 * ServiceCreditReportCard — Relatório de créditos de SERVIÇOS.
 * Espelha VehicleCreditReportCard.
 */
...
export function ServiceCreditReportCard() {
  const { user } = useAuth();

  const { data: balance = 0 } = useQuery({
    queryKey: ["se-report-balance", user?.id],
    enabled: !!user?.id,
  ...
  const { data: debits = [], isLoading: debitsLoading } = useQuery({
    queryKey: ["se-report-debits", user?.id],
    enabled: !!user?.id,
    refetchInterval: 20_000,
    queryFn: async () => {
  ...
  const { data: purchases = [], isLoading: purchasesLoading } = useQuery({
    queryKey: ["se-report-purchases", user?.id],
    enabled: !!user?.id,
    refetchInterval: 30_000,
    queryFn: async () => {
      const { data } = await (supabase.from("service_credit_purchases") as any)
    ...
    <section className="space-y-6">
      <div className="space-y-3">
        ...
        <p className="text-[#A7B0BE] font-bold uppercase text-[10px] tracking-[0.2em] ml-16">
          Débitos de navegação, interesses e compras de pacotes (serviços)
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-[#1B1F24] border-[#2A3038]">
          <CardContent className="p-5 space-y-1">
            <p className="text-[10px] font-black uppercase tracking-widest text-[#A7B0BE] flex items-center gap-1.5">
              <Wallet className="w-3.5 h-3.5 text-violet-500" /> Saldo atual
            </p>
...
export default ServiceCreditReportCard;
